...
// ════════════════════════════════════════════════
// REVE PROVIDER + HOOK
// L'app détermine son seed du jour au montage, mémorise les collages tirés,
// et fournit la composition à toute la hiérarchie.
// ════════════════════════════════════════════════
...
function composerSeance(seed: number): SeanceReve {
  // Le rng est créé une fois par seed et partagé pour toute la séance
  const rng = mulberry32(seed)

  // Filtre les collages déjà vus
  const pool = filtrerMemoire(COLLAGES)
  // Pioche 6 collages (un par grand écran + réserves)
  ...
  return {
    ...
    retirer: () => {}, // sera attribué dans le provider
  }
}
...
/** Hook : retourne la séance rêve courante. Renvoie null si pas de Provider. */
...
// ════════════════════════════════════════════════
// COMPOSANTS DE DÉCOR
// À placer dans une page React avec position: relative
// ════════════════════════════════════════════════

interface DecorProps {
  /** Quel écran on décore — détermine le placement par défaut. */
  variant?: 'accueil' | 'config' | 'jeu' | 'fin' | 'biblio' | 'detail'
  /** Index du collage à utiliser (0..5) — par défaut, varie selon la variante. */
  collageIndex?: number
  /** Cacher le label du collage. */
  hideLabel?: boolean
  /** Masquer la marginalia. */
  hideMarginalia?: boolean
  /** Masquer le dérèglement. */
...
/** Décore l'écran avec collage + marginalia + dérèglement, selon le contexte. */
...
/** Signature du rêve (« rêve № 17 ») à placer en bas. Optionnel. */
