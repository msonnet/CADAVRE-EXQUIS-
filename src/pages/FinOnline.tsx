        ...
        {!revealReady && (
          ...
          >
            ...
            <motion.div style={{ position: 'relative', zIndex: 1 }} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7 }}>
              <div style={{ ...mono, fontSize: 12, color: accent, letterSpacing: '0.28em', marginBottom: 20, opacity: 0.8 }}>— {players.length} VOIX —</div>
              ...
              <div style={{ marginBottom: 28 }}>
                {loadingDessin && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '32px 0' }}>
                    ...
                    <span style={{ ...mono, fontSize: 12, color: accent }}>Assemblage du dessin…</span>
                  </div>
                )}
                ...
                {imageAssemblee && !loadingDessin && (
                  <div style={{ marginBottom: 20 }}>
                    ...
                    {texteVision ? (
                      <>
                        <div style={{ ...mono, fontSize: 12, color: accent, fontWeight: 700, letterSpacing: '0.22em', marginBottom: 10 }}>— LECTURE SURRÉALISTE —</div>
                          ...
                          <div key={c.case_index} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <div style={{ width: 28, height: 28, borderRadius: 2, overflow: 'hidden', flexShrink: 0, border: `1px solid ${accent}30` }}>
                              {p?.avatar_url ? <img src={p.avatar_url} alt={p.pseudo} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                : <div style={{ width: '100%', height: '100%', background: `${accent}20`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontFamily: "'Bodoni Moda', serif", fontSize: 12, color: accent, fontWeight: 900 }}>{p?.pseudo[0]?.toUpperCase() ?? '?'}</span></div>}
                            </div>
            ...
            {room.host_id === user?.id && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
                <button onClick={rejouerEnsemble}
                  style={{ width: '100%', background: 'transparent', color: encre, ...mono, fontSize: 13, textTransform: 'uppercase', padding: '0.85em 1.8em', border: `1px solid ${encre}40`, cursor: 'pointer', marginTop: 8 }}>
                  ↻ REJOUER ENSEMBLE
                </button>
              </motion.div>
            )}
