  ...
  if (!poeme) {
    return (
      <PageTransition className="page-carnet flex flex-col items-center justify-center min-h-dvh safe-top safe-bottom">
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '0.9375rem', color: encre, opacity: 0.85, textAlign: 'center' }}>
          Aucun poème en cours.
        </p>
        <button
          onClick={() => navigate('/config')}
          style={{ marginTop: 32, background: accent, color: btnText, ...mono, fontSize: '0.9375rem', textTransform: 'uppercase', padding: '0.9em 1.8em', border: 'none', cursor: 'pointer' }}
        >
          Nouvelle partie
        </button>
      </PageTransition>
    ...
    <>
      ...
      <AnimatePresence>
        {!revealReady && poeme && (
          ...
          >
            ...
            >
              <div style={{ ...mono, fontSize: '0.875rem', color: accent, letterSpacing: '0.28em', marginBottom: 20, opacity: 0.8 }}>
                — {voixCount} VOIX —
              </div>
              <div style={{
            ...
            <motion.div
              style={{ position: 'relative', zIndex: 1, fontSize: '1.125rem', color: accent }}
      ...
      <AnimatePresence>
        {pleinEcran && illustrationUrl && (
          ...
          >
            ...
            />
            {labelStyle && (
              <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: '0.875rem', letterSpacing: '0.18em', color: '#e8d4b8', opacity: 0.75, marginTop: 12 }}>
            ...
            <button
              ...
              style={{ position: 'absolute', top: 20, right: 20, fontFamily: "'Outfit', sans-serif", fontSize: '0.9375rem', letterSpacing: '0.18em', color: '#e8d4b8', opacity: 0.85, background: 'none', border: 'none', cursor: 'pointer' }}
            >
              ✕ FERMER
            </button>
          </motion.div>
      ...
      <div style={{ position: 'relative', zIndex: 10 }} className="flex flex-col flex-1">
        ...
        >
          ...
          {/* Card footer */}
          <div style={{ ...mono, fontSize: '0.875rem', color: encre, opacity: 0.75, marginTop: 14, paddingTop: 8, borderTop: `0.5px solid ${encre}20` }}>
          ...
          >
            ...
            <div className="flex justify-between items-center">
              {labelStyle && !generatingIllustration && (
                <span style={{ ...mono, fontSize: '0.875rem', color: encre, opacity: 0.75 }}>{labelStyle.toUpperCase()}</span>
              )}
              {!generatingIllustration && (
                <button
                  onClick={() => setPleinEcran(true)}
                  aria-label="Agrandir l'illustration"
                  style={{ ...mono, fontSize: '0.875rem', color: accent, opacity: 0.8, background: 'none', border: 'none', cursor: 'pointer', marginLeft: 'auto' }}
                >↗ AGRANDIR</button>
              )}
            </div>
            ...
            {promptVisuel && !generatingIllustration && (
              <div className="w-full">
                <button
                  ...
                  style={{ ...mono, fontSize: '0.875rem', color: encre, opacity: 0.75, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                >
        ...
        <AnimatePresence>
          {generatingIllustration && (
            ...
            >
              ...
              >✦</motion.span>
              <p style={{ ...mono, fontSize: '0.875rem', color: encre, opacity: 0.8 }}>{labelStyle?.toUpperCase()} EN COURS…</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          ...
          >
            ...
            <span aria-hidden style={{ fontSize: '0.875rem', opacity: 0.85 }}>→</span>
          </button>
        </motion.div>
        ...
        >
          <button
            ...
            style={{ ...mono, fontSize: '0.9375rem', color: parlant ? accent : encre, opacity: parlant ? 0.9 : 0.5, background: 'none', border: 'none', cursor: 'pointer' }}
          >
            {parlant ? '◾ RÉCITER' : '— RÉCITER —'}
          </button>
          <button
            onClick={partager}
            style={{ ...mono, fontSize: '0.9375rem', color: partageOk ? accent : encre, opacity: partageOk ? 0.9 : 0.5, background: 'none', border: 'none', cursor: 'pointer' }}
          >
            {partageOk ? '✓ COPIÉ' : '— PARTAGER —'}
          </button>
          <button
            onClick={() => setActiveSection(s => s === 'coutures' ? null : 'coutures')}
            style={{ ...mono, fontSize: '0.9375rem', color: activeSection === 'coutures' ? accent : encre, opacity: activeSection === 'coutures' ? 0.9 : 0.5, background: 'none', border: 'none', cursor: 'pointer' }}
          >
            — COUTURES —
          </button>
          <button
            onClick={() => setActiveSection(s => s === 'image' ? null : 'image')}
            style={{ ...mono, fontSize: '0.9375rem', color: activeSection === 'image' ? accent : encre, opacity: activeSection === 'image' ? 0.9 : 0.5, background: 'none', border: 'none', cursor: 'pointer' }}
          >
            — IMAGE —
          </button>
        </motion.div>
              ...
              {poeme.cases.map((c, i) => {
                ...
                return (
                  <div key={i} style={{ borderLeft: `2px solid ${accent}30`, paddingLeft: 12, paddingTop: 2, paddingBottom: 2 }}>
                    <p style={{ ...mono, fontSize: '0.875rem', color: accent, opacity: 0.7, marginBottom: 3 }}>
                    ...
                    <p style={{ fontFamily: "'Cormorant Garamond', serif", color: encre, fontSize: '1.0625rem', lineHeight: 1.4 }}>
                      {c.texte}
                    </p>
                  </div>
                )
        ...
        <AnimatePresence>
          {activeSection === 'image' && (
            ...
            >
              ...
              <div className="mb-4">
                ...
                {promptLibre.trim() && (
                  <button
                    ...
                    style={{ ...mono, fontSize: '0.875rem', color: accent, background: 'none', border: `0.5px solid ${accent}50`, padding: '8px 12px', cursor: 'pointer', marginTop: 8, width: '100%' }}
                  >
              ...
              {erreurIllustration && (
                <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '0.9375rem', color: accent, opacity: 0.7, textAlign: 'center', marginBottom: 10 }}>
                  {erreurIllustration}
                </p>
              )}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {illustrationUrl && (
                    <button
                      ...
                      style={{ ...mono, fontSize: '0.875rem', color: encre, opacity: 0.8, background: 'none', border: `0.5px solid ${encre}20`, padding: '8px', cursor: 'pointer' }}
                    >
                      ↺ RELANCER
                    </button>
                  )}
              ...
              {promptVisuel && !generatingIllustration && (
                <div className="flex flex-col items-center mt-3">
                  <button
                    onClick={() => setPromptVisible(v => !v)}
                    style={{ ...mono, fontSize: '0.875rem', color: encre, opacity: 0.7, background: 'none', border: 'none', cursor: 'pointer' }}
                  >
                    {promptVisible ? '↑ MASQUER LE PROMPT' : '→ VOIR LE PROMPT IA'}
                  </button>
                  {promptVisible && (
                    <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '0.875rem', color: encre, opacity: 0.85, marginTop: 6, textAlign: 'center', lineHeight: 1.5, maxWidth: 280 }}>
                      {promptVisuel}
                    </p>
                  )}
                </div>
        ...
        >
          <button
            onClick={() => navigate('/config')}
            style={{ ...mono, fontSize: '0.9375rem', color: encre, opacity: 0.75, background: 'none', border: 'none', cursor: 'pointer' }}
          >
            — NOUVELLE PARTIE —
          </button>
        </motion.div>
